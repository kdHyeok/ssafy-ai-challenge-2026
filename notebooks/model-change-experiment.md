# Multimodal VQA 성능 개선 여정: Baseline에서 Qwen3-VL-8B까지

이 문서는 baseline 코드를 출발점으로 모델만 바꾸며 실행한 초기 모델 변경 실험을 정리합니다. 목적은 모든 하이퍼파라미터를 완전히 통제한 ablation이 아니라, Hugging Face 모델 카드와 baseline 코드를 기준으로 모델을 교체했을 때 어떤 모델이 대회 객관식 VQA 포맷에 더 잘 맞는지 빠르게 확인하는 것이었습니다.
...
모델 크기와 VQA 적합성이 좋아질수록 점수는 전반적으로 상승했습니다. `google/gemma-4-E4B-it`는 최신 모델이었지만 대회 당일 출시 모델이라 레퍼런스와 튜닝 사례가 부족했고, 실제 점수도 `Qwen/Qwen3-VL-8B-Instruct`보다 근소하게 낮았습니다. 그래서 이후 실험은 Qwen3-VL-8B를 기준으로 잡고, 같은 Qwen3-VL 구조에서 32B로 확장했습니다.

## Stage 1 — Baseline: Qwen2.5-VL-3B-Instruct

대표 노트북: `Baseline_0.70279.ipynb`
...
이 baseline은 학습 데이터 200개만 사용하고, 이미지를 384 고정 해상도로 처리했습니다. 이 설정은 빠르게 end-to-end 흐름을 확인하기에는 좋았지만, 전체 데이터 다양성과 Qwen-VL의 동적 해상도 장점을 충분히 쓰지 못했습니다.
...
MiniCPM-V-4_5 모델 카드는 Qwen3-8B와 SigLIP2-400M 기반의 멀티모달 모델이며, 단일 이미지, 다중 이미지, 비디오 이해, OCR, document parsing을 강조합니다.

Baseline 코드에서 바꿔야 했던 핵심은 다음과 같습니다.
...
Gemma는 Qwen chat template과 토큰 구성이 다르기 때문에, baseline의 Qwen 전용 message format을 그대로 쓰면 안정적인 a/b/c/d 출력이 어렵습니다. 객관식 VQA에서는 모델 성능뿐 아니라 최종 출력이 정확히 한 글자로 떨어지는지도 중요하므로, system/user message와 answer parsing을 Gemma 방식에 맞게 정리해야 했습니다.

결과는 0.82815였습니다. 최신 모델이었지만 `Qwen/Qwen3-VL-8B-Instruct`보다 낮았습니다. 면접에서 이 질문을 받는다면 다음처럼 설명할 수 있습니다.

> 최신 모델이라고 해서 대회 점수가 바로 높아지는 것은 아니었습니다. Gemma 4 E4B는 대회 당일 출시된 모델이라 레퍼런스와 튜닝 사례가 부족했고, 객관식 VQA에서 중요한 chat template, 프롬프트, a/b/c/d 출력 파싱을 충분히 안정화하기 어려웠습니다. 실제 점수도 Qwen3-VL-8B보다 낮았기 때문에, 대회 기간 안에서는 더 안정적으로 점수가 나온 Qwen3-VL-8B를 채택했습니다.
...
6. loss 이동평균과 validation 흐름을 같이 확인한다.
...
1. 모델 크기는 성능 상한에 영향을 줬습니다. Qwen2.5-VL-3B baseline에서 MiniCPM-V-4_5, Gemma 4 E4B, Qwen3-VL-8B로 바꾸며 점수가 전반적으로 상승했습니다.
...
4. 학습 파라미터는 loss만 보고 결정하지 않았습니다. answer 분포, validation 흐름, 실제 제출 점수를 같이 확인했습니다.
5. Qwen3-VL-8B가 가장 안정적인 초기 선택이었기 때문에 Qwen3-VL-32B 확장이 자연스러운 다음 단계가 되었습니다.
