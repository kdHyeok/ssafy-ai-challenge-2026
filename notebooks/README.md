...
이 디렉토리에는 VQA 대회 실험 순서를 설명하는 노트북과 실험 문서가 있습니다. baseline, 모델 변경, EDA, 32B LoRA 학습, 선택적 재추론, soft ensemble, detection/TTA 확장 실험 순서로 구성했습니다.

## 파일 목록

| 파일 | 역할 | 내용 |
|---|---|---|
| `Baseline_0.70279.ipynb` | 초기 baseline | `Qwen/Qwen2.5-VL-3B-Instruct` 기반 4bit LoRA baseline. 384 고정 해상도, train 200개 샘플, 제출 CSV 생성까지 end-to-end 실행 |
...
- 역할: 데이터 로드, 이미지 전처리, 프롬프트 구성, fine-tuning, submission 생성 실행
- 결과: 0.70279

### 2. 모델 변경 실험

관련 파일: `model-change-experiment.md`
...
- 정답 분포와 선택지 편향 확인
- image resize/padding/crop이 count/material 문제에 영향을 주는지 후속 실험 후보로 남김

EDA는 성능 향상 claim이 아니라 32B 학습, confidence routing, Thinking 재추론, detection crop을 고른 이유를 설명하는 자료입니다.

### 4. 학습 파라미터 조정

수업 메모와 직접 비교 실험에서 확인한 내용은 다음과 같습니다.
...
- 학습 전략: train 전체 사용, dev pseudo-label 후보 사용, augmentation, sequence length 점검
...
이 전략은 hard voting이 아니라 확률을 가중 합산한 soft ensemble입니다. 최고점 0.94402보다 낮았기 때문에, 최고점에는 못 미친 후반 비교 실험입니다.
...
- 원본 이미지와 crop 이미지를 VQA 추론 입력에 같이 사용
- Qwen3-VL-32B LoRA와 Qwen3.5-27B의 원본/좌우반전 TTA 확률 결합
- 결과: 0.93102

이 실험은 VLM이 재활용품 객체를 세밀하게 인식하지 못하는 문제를 보완하려고 한 후반 실험입니다. detector 결과를 pseudo-label이나 fine-tuning label로 사용하지 않았고, 추론 때 crop/TTA 보조 입력으로만 썼습니다.
...
따라서 이 저장소에서는 Falcon을 최종 제출 파이프라인이 아니라 다음 개선 아이디어로 남깁니다.

## 점수 해석

노트북명의 점수는 해당 실험의 실제 기록값입니다. 일부 제출은 로컬 저장명과 대회 업로드명이 다를 수 있어, README와 이 문서에서는 점수, 실험 방법, 선택 이유를 함께 적었습니다.
...
대회 원본 데이터와 학습된 모델 weight는 저장소에 포함하지 않습니다. 노트북은 실험 로직과 프로젝트 진행 과정을 설명하기 위한 자료입니다.
