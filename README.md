...
재활용품 이미지와 객관식 질문을 입력으로 받아 정답 선택지를 예측하는 VQA 대회 프로젝트입니다. Baseline 코드에서 시작해 모델 교체, 학습 파라미터 조정, Qwen3-VL-32B LoRA 학습, soft ensemble, detection/TTA를 실험했습니다. 최종 제출 파이프라인은 `notebooks/final_score_0.94402.ipynb`입니다. selective Thinking 실험은 실패 기록 문서로만 남겼습니다.
...
- 32B 학습 실험: `train_0.9333.ipynb`의 실제 제출 점수는 0.93141
...
3. 병행해서 EDA로 dev 응답 불일치, 질문 유형, count/material 문제를 확인했고, 후속 실험 우선순위를 잡았습니다.
4. 학습 로그와 비교 실험 결과를 바탕으로 learning rate, scheduler, optimizer, label smoothing, LoRA rank/alpha, batch size를 조정했습니다.
5. `Qwen/Qwen3-VL-32B-Instruct` 학습으로 확장했고, `train_0.9333.ipynb`의 실제 제출 점수는 0.93141입니다.
6. Qwen3-VL-32B와 Qwen3.5-27B를 섞는 soft ensemble을 시험해 0.94363까지 올렸습니다.
7. 후반에는 Grounding DINO, Florence-2, crop, TTA, 듀얼 VQA 결합을 붙인 detection/TTA 파이프라인을 최종 제출 흐름으로 사용했습니다.
8. selective Thinking 후처리 아이디어 실험은 실패 기록으로만 남기고, 노트북은 제외했습니다.
...
`experimental/EDA.ipynb`에서는 train/dev/test 구조, dev 응답 불일치, 질문 유형, 정답 분포, count/material 문제를 먼저 확인했습니다. 이 단계는 모델 변경과 병행했고, 이후 32B 학습·soft ensemble·detection/TTA 우선순위를 잡는 근거로 썼습니다.

### 4. 학습 파라미터 조정

학습 파라미터는 학습 로그와 비교 실험 결과를 함께 보며 조정했습니다. 초기 loss, 작은 샘플 과적합 여부, learning rate 후보, scheduler, label smoothing, optimizer, answer 분포를 같이 보고 설정을 바꿨습니다.

### 5. Qwen3-VL-32B LoRA 학습

`train_0.9333.ipynb`는 `Qwen/Qwen3-VL-32B-Instruct`를 Unsloth와 LoRA로 학습한 노트북입니다. 실제 제출 점수는 0.93141입니다. 이 단계부터 answer와 함께 a/b/c/d 확률을 저장하는 구조가 붙었습니다.
...
`final_score_0.94402.ipynb`는 Grounding DINO와 Florence-2로 관심 객체 crop을 만들고, 원본 이미지와 crop 이미지를 같이 넣어 VQA 추론한 뒤 TTA와 듀얼 모델 확률을 결합한 최종 제출 파이프라인입니다. 자세한 기록은 `docs/detection-tta-0.94402.md`와 `docs/detection-retrospective.md`에 따로 적었습니다.

### 8. selective Thinking 실패 실험

여러 제출의 답이 갈리는 샘플만 Thinking 모델로 다시 보는 후처리 아이디어를 시험했습니다. 다만 로그 파싱과 결과 저장이 불안정해, 노트북 대신 실패 실험 기록만 남깁니다.
...
| 구분 | 결과 요약 | 설명 |
|---|---:|---|
| 최고 제출 | 0.94402 | `final_score_0.94402.ipynb` 최종 제출 파이프라인 |
...
- 김동혁: 아래 `기여한 내용` 섹션에서 따로 설명합니다.

## 기여한 내용

이 저장소에서 맡은 실험과 문서화 범위는 아래처럼 묶을 수 있습니다.

| 영역 | 기여 내용 | 관련 파일 |
|---|---|---|
| EDA 및 데이터 분석 | dev 응답 불일치, 질문 유형, count/material 문제를 분석하고 후속 실험 우선순위를 잡음 | `notebooks/experimental/EDA.ipynb` |
| 모델 변경 실험 | baseline 코드에서 모델 카드에 맞춰 MiniCPM-V-4_5, Gemma 4 E4B, Qwen3-VL-8B를 실행하고 비교 | `docs/model-change-experiment.md` |
| 학습 파라미터 조정 | 학습 로그와 비교 실험 결과를 바탕으로 learning rate, scheduler, label smoothing, optimizer, LoRA 설정을 조정 | `docs/model-change-experiment.md`, `notebooks/train_0.9333.ipynb` |
| detection/TTA 기록 | detection crop, TTA, 듀얼 VQA 결합 실험과 객체 인식 회고를 함께 설명 | `docs/detection-tta-0.94402.md`, `docs/detection-retrospective.md`, `notebooks/final_score_0.94402.ipynb` |
| 문서 구조화 | 노트북 역할과 세부 설명이 맞도록 README와 하위 문서 구조를 맞춤 | `README.md`, `docs/`, `notebooks/README.md` |
| 실패 실험 기록 | selective Thinking 실패 실험을 노트북 대신 문서로 남김 | `docs/selective-thinking-failed-experiment.md`, `notebooks/experimental/README.md` |
...
`train_0.9333.ipynb`처럼 파일명에 붙은 숫자와 실제 제출 점수가 다른 경우가 있었습니다. 이 저장소에서는 실제 제출 점수를 기준으로 설명했습니다.

### 실패 실험도 분리해서 남겨야 함

제거한 selective Thinking 실험은 파일명만 보면 대표본처럼 보일 수 있었지만, 로그 파싱과 결과 저장이 불안정했습니다. 그래서 최종 제출 흐름과 실패 실험 기록을 분리했습니다.

### detection/TTA는 후반 보정이 아니라 최종 제출 흐름으로 다시 봐야 함

detection/TTA는 후반 보조 실험이 아니라, 실제 최종 제출 흐름으로 보는 편이 전체 실험 순서와 맞았습니다.
...
학습된 모델 weight와 checkpoint는 포함하지 않습니다. 공개 노트북은 실험 순서와 주요 코드를 보여주기 위한 파일입니다. `data/`에는 CSV 구조와 이미지 경로를 설명하기 위한 split별 10행 샘플과 이미지 샘플만 두었습니다. 공개 레포만으로 exact score 재현을 보장하지는 않지만, 모델명, 학습 코드, 추론 로직, 확인된 제출 점수를 볼 수 있게 구성했습니다.
